...
Selenium等の外部OSSのインストールを必要とせず、任意のフォルダに配置するだけで職場のPCでも利用できる「完全ポータブル環境」で構築されています。VBAを司令塔とし、PowerShell 5.1経由でMicrosoft Edgeのレンダリングコア（WebView2）を制御します。<br>
...
```text
    ...
    +-------------------------------------------------------------+
                ...
                v                                   v
    +-------------------------------------------------------------+
    |             ターゲットWebページ (DOM / iframe / Shadow DOM)   |
    +-------------------------------------------------------------+
```  
...
**マニュアル** 汎用RPA操作エンジン 内部開発・運用仕様書
QIITA
【VBA×PowerShell】多段iframeとShadow DOMを透過する探索アルゴリズム
【VBA×WebView2】脱IE！（はじめの一歩）、レスポンシブWebの非表示メニューを賢くクリック
【VBA × WebView2】脱IE! 「Edge内蔵PDFビューア」からPDFをダウンロードする
